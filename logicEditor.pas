...
begin
  ...
  begin
    ...
    curCableBeingCreated.path.Add(TGate(Sender).mostRecentClick);
  ...
  begin
    ...
    curCableBeingCreated.path := TList<TPoint>.Create();
  end;
end;
...
begin
  ...
  begin
    if curCableBeingCreated.Path.Count = 0 then
      curCableBeingCreated.path.Add(TPoint.Create(curWireBeingCreated.initX, curWireBeingCreated.initY));

    prevX := curCableBeingCreated.path.Last.X;
    prevY := curCableBeingCreated.path.Last.Y;

    if Max(prevX, X) - Min(prevX, X) > Max(prevY, Y) - Min(prevY, Y) then
      curCableBeingCreated.path.Add(TPoint.Create(X, prevY))
    else
      curCableBeingCreated.path.Add(TPoint.Create(prevX, Y));

    WriteLn('Added path');
    WriteLn(curCableBeingCreated.path.Count);
  end;
end;
